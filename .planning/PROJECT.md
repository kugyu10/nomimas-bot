...
- LINE dev/prod のチャネルID・チャネルシークレットはenvファイルに格納済み。**チャネルアクセストークンはユーザー発行待ち**
- **X OAuthアプリは未作成** — Phase 3の認証は、プロバイダー設定を後から差し込める構造にする（それまでmock/テストプロバイダーでのフォールバック可）
...
- **Dependencies**: LINEチャネルアクセストークン未発行・X OAuthアプリ未作成 — 実機検証はモック/テストで代替し、設定差し込みのみで本番化できる構造にする
